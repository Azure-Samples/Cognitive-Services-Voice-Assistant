...
url="https://${region}.commands.speech.microsoft.com/v1.0/apps/${appid}/slots/default/languages/${language}/model"
authHeader="Ocp-Apim-Subscription-Key: $subscriptionkey"

curl -s -X PUT ${url} -H "$authHeader" -H "Content-Type: application/json" -d @"$file" -o ImportResult.json
result=$(cat ImportResult.json)

if [[ "$result" == *"error"* ]]; 
then
    echo "Error while importing: $result"
    exit 1
fi
